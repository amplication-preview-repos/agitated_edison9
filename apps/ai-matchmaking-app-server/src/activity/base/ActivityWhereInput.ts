/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ActivitySummaryListRelationFilter } from "../../activitySummary/base/ActivitySummaryListRelationFilter";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";
import { ConferenceWhereUniqueInput } from "../../conference/base/ConferenceWhereUniqueInput";
import { StringNullableFilter } from "../../util/StringNullableFilter";
import { StringFilter } from "../../util/StringFilter";

@InputType()
class ActivityWhereInput {
  @ApiProperty({
    required: false,
    type: () => ActivitySummaryListRelationFilter,
  })
  @ValidateNested()
  @Type(() => ActivitySummaryListRelationFilter)
  @IsOptional()
  @Field(() => ActivitySummaryListRelationFilter, {
    nullable: true,
  })
  activitySummaries?: ActivitySummaryListRelationFilter;

  @ApiProperty({
    required: false,
    type: () => ConferenceWhereUniqueInput,
  })
  @ValidateNested()
  @Type(() => ConferenceWhereUniqueInput)
  @IsOptional()
  @Field(() => ConferenceWhereUniqueInput, {
    nullable: true,
  })
  conference?: ConferenceWhereUniqueInput;

  @ApiProperty({
    required: false,
    type: StringNullableFilter,
  })
  @Type(() => StringNullableFilter)
  @IsOptional()
  @Field(() => StringNullableFilter, {
    nullable: true,
  })
  description?: StringNullableFilter;

  @ApiProperty({
    required: false,
    type: StringFilter,
  })
  @Type(() => StringFilter)
  @IsOptional()
  @Field(() => StringFilter, {
    nullable: true,
  })
  id?: StringFilter;

  @ApiProperty({
    required: false,
    type: StringNullableFilter,
  })
  @Type(() => StringNullableFilter)
  @IsOptional()
  @Field(() => StringNullableFilter, {
    nullable: true,
  })
  name?: StringNullableFilter;
}

export { ActivityWhereInput as ActivityWhereInput };
